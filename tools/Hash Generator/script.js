document.addEventListener('DOMContentLoaded', function () {
    ...
    // Algorithm selection
    md5Btn.addEventListener('click', () => selectAlgorithm('md5'));
    sha256Btn.addEventListener('click', () => selectAlgorithm('sha256'));
    ...
    function selectAlgorithm(algorithm) {
        ...
        if (algorithm === 'md5') {
            md5Btn.classList.add('active');
        } else {
            sha256Btn.classList.add('active');
        }
    }
    ...
    generateBtn.addEventListener('click', async function() {
        ...
        try {
            ...
            if (selectedAlgorithm === 'md5') {
                ...
                hashAlgorithm.textContent = 'MD5 Hash';
            } else {
                hash = await generateSHA256(text);
                hashLength.textContent = `${hash.length} characters`;
                hashAlgorithm.textContent = 'SHA-256 Hash';
            }
    ...
    copyBtn.addEventListener('click', async function() {
        try {
            await navigator.clipboard.writeText(hashDisplay.textContent);
